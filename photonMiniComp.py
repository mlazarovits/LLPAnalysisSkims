...
from ComparisonPlotting import create_comparison_canvas, dist, parse_data_name, create_comparison_canvas_2d
from RJRAnalysis_v46 import RJRAnalysis
...
def AddHists(df, hists, hists2d, dfs, branches, histname = ""):
	#inclusive photons
	...
	eta_time = df.Histo2D((branches["PhoWTime"]+"_"+branches["PhoEta"]+histname,branches["PhoWTime"]+"_"+branches["PhoEta"]+histname,50,-20,20,50,-3,3),branches["PhoWTime"],branches["PhoEta"])
	hists2d.append(eta_time)
...
for file in files:
	...
	hists, dfs = AddHists(df, hists, hists2d, dfs, branch_map, histname)
	#isolation cuts
	...
	hists, dfs = AddHists(df_isophos, hists, hists2d, dfs, branch_map, histname)
	if "MINI" in file:
		hists_mini[histname] = hists
		hists_mini_2d[histname] = hists2d
	else:
		hists_aod[histname] = hists
		hists_aod_2d[histname] = hists2d
	df0.Report().Print()
...
#write 2D hists
for histname in hists_mini_2d:
	print("histname",histname)
	hists = hists_mini_2d[histname]
	for hist in hists:
		name = hist.GetName()
		print("Writing canvas",name)
		canvas, objs = create_comparison_canvas_2d(name, hist, histname, globallabel = "MINI")
		canvas.Write()
for histname in hists_aod_2d:
	print("histname",histname)
	hists = hists_aod_2d[histname]
	for hist in hists:
		name = hist.GetName()
		print("Writing canvas",name)
		canvas, objs = create_comparison_canvas_2d(name, hist, histname, globallabel = "AOD")
		canvas.Write()
